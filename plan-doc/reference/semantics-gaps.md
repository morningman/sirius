...
| G-31 | **Sirius 引擎：MARK join 在定尺寸前被轮询会 `std::terminate`** | Doris 把 `NOT IN (子查询)` 规划成 `NULL_AWARE_LEFT_ANTI_JOIN`，翻译成 `LeftMark` + `Filter[not(mark)]`（Q16）；FE 的 join 顺序把它放在上层 inner join 的 **probe 侧** | Sirius 的 `sirius_physical_hash_join::get_next_task_hint` 在 STANDARD 模式下调 `refresh_cross_schedule`，后者对 MARK join 直接 throw（"MARK join must run in BUILD_PROBE mode"）；上层 inner join 的 build（part）先跑完后沿 `WAITING_FOR_INPUT_DATA` 走进 probe 管线的源算子 = 这个 MARK join，而它两侧 partition 都还没见过数据、没协商出 BUILD_PROBE → 异常从 task creator 线程抛出 → **整个进程 terminate**（BE 直接死，FE 报 `UNAVAILABLE: io exception`）。透明路径的 Q16 因为 DuckDB 优化器把 MARK join 放在最顶层（build = 第一个 scan）碰不到 | ✅ **引擎侧已修（fork，09-19，待上游 PR）**：`get_next_task_hint` 对未定尺寸的 MARK join 返回「等 build 生产者」而不是 throw，让 task creator 先去跑 build 侧，其 partition 定尺寸后进入 BUILD_PROBE。Q16 18,314 行一致；透明路径 22/22 回归通过。**空 build（子查询无行）时两侧 partition 都不会协商**（非驱动侧直接 `_num_partitions=1`）——修前 throw、修后会等 build 生产者；透明路径的空 build 探针（`NOT IN (… LIKE 'zzz%')`）返回 800000 正常，A0 语料未覆盖，留作 MVP-A 的探针 | `src/op/sirius_physical_hash_join.cpp` `get_next_task_hint`；`src/planner/sirius_plan_aggregate.cpp` 的 `can_feed_dense_count_join` 注释早就点名了这个坑 |
| G-32 | **Sirius 引擎：mixed join（等值 + 不等值）的不等值条件里有 DECIMAL cast / 算术** | Doris 把关联子查询规划成 inner join，`other_join_conjuncts` = `cast(l_quantity as DECIMAL(38,5)) < 0.2 * avg_qty`（Q17）、`ps_availqty > 0.5 * sum_qty`（Q20） | DuckDB 优化器（`PushdownInnerJoin` → `ExtractJoinConditions`）把「左引用 vs 右表达式」的比较一律收成 join condition，翻译器就算发成 join 上方的 Filter 也会被收回去；Sirius 的 planner 只把**等值**键的复杂表达式物化成投影列，不等值侧留给 cuDF AST 谓词内联求值，而 cuDF AST 既不能 cast 到 DECIMAL 也不做 DECIMAL 算术 → 执行期 "failed to translate mixed join inequality conditions to cuDF AST predicate"。透明路径的 Q17/Q20 走 DELIM_JOIN，这个比较留在 join 上方的 FILTER 里，碰不到 | ✅ **引擎侧已修（fork，09-19，待上游 PR）**：`materialize_expression_join_keys` 对不等值条件的两侧也按「AST 能否求值」判定，非引用 / 非 AST 支持的 cast 一律物化成投影列，谓词变成两列比较（DECIMAL128 同 scale 比较 cuDF AST 支持）。Q17 `avg_yearly` 与 Q20 186 行一致 | `src/planner/sirius_plan_comparison_join.cpp` `materialize_expression_join_keys` / `is_trivial_key_side`；`src/expression_evaluator/gpu_expression_translator.cpp` cast 只支持 INT64/UINT64/FLOAT64、function 有 decimal-propagation guard |

## 待验证（验证前一律拒绝）

| # | 主题 | 怀疑点 | 怎么验 |
|---|---|---|---|
| G-19 | **DECIMAL 运算结果精度推导** | 两侧的精度/scale 推导规则大概率不同。这是**最高风险项** —— 影响 TPC-H 几乎每条查询的金额计算。**09-18 对照 DuckDB 源码**：乘法两边一致（p1+p2 封顶 38，s1+s2；Q1 `(15,2)*(16,2)→(31,4)`、`(31,4)*(16,2)→(38,6)`）；`sum(DECIMAL(p,s))` 两边都是 `(38,s)`；**`avg(DECIMAL)` DuckDB 返回 DOUBLE**（`avg.cpp` `BindDecimalAvg`），Doris 返回 `DECIMAL128I(38,4)`；加减 DuckDB 会多留 1 位精度；除法规则未核对。消费端忽略 `ScalarFunction.output_type`，DuckDB 自己推类型。**09-18 晚 CPU 差分（A0.1）**：tuple 边界的显式 `Cast` 在消费端落地了——Q1 plan 结果类型 `avg_* DECIMAL(38,4)`、Q8 `o_year SMALLINT`、Q12 `BIGINT`，与 Doris 声明一致；22 条数值全部在 `max(1e-9 相对, 半 ulp)` 内（半 ulp 只为 avg 的 DECIMAL(38,4) 舍入）。**09-19 GPU 差分（A0.5，T4）**：22 条与 DuckDB 基线一致，**唯一差异是 `CAST(DOUBLE AS DECIMAL)` 的最后一位**——Sirius 用 cudf 的 float→fixed_point 转换，它**截断**不舍入（`cudf/fixed_point/detail/floating_conversion.hpp`："We have chosen not to round"），DuckDB 舍入：Q1 `avg_disc` GPU `0.0499` vs CPU `0.0500`（真值 0.049985…），`avg_qty`/`avg_price` 同样，共 7 个值差 1 ulp；乘法/sum/decimal 比较全部精确。Sirius 没有 `round`/`floor`/`ceil`（`function_id.hpp`），翻译器无法先舍入再 cast | 翻译器在 tuple 边界（projection/agg 输出）按 Doris 声明类型显式 `Cast`（P1.3 已做，CPU 差分已验）；**GPU 上的 DOUBLE→DECIMAL 截断按 1 ulp 容差接受**：`validate_tpch_results.py --ulps 1`（`run-tpch.sh` 执行模式默认），结论里标出靠这条容差过的值的个数。要逐位一致得等引擎侧 cast 舍入或 `round` 支持；Doris 真 BE 的 `avg(DECIMAL)` 是 decimal 除法舍入，与 DuckDB 一致 |
